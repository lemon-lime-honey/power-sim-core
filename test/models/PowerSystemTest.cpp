...
TEST_F(PowerSystemTest, ThrowsWhenAddingLineWithInvalidBus) {
  auto invalidLine = std::make_shared<Line>("Line1", 1, 99, 22.9, 0.1, 0.2, 0.0,
                                            0.0, 0.3, 0.6, 0.0, 0.0);
  EXPECT_THROW(sys.addLine(invalidLine), std::invalid_argument);
}

TEST_F(PowerSystemTest, RetrievesConnectedEquipmentAccurately) {
  auto line = std::make_shared<Line>("Line1", 1, 2, 22.9, 0.1, 0.2, 0.0, 0.0,
                                     0.3, 0.6, 0.0, 0.0);
  sys.addLine(line);
...
TEST_F(PowerSystemTest, MapsProtectionDevicesCorrectly) {
  ...
  sys.addLine(std::make_shared<Line>("Line_A", 1, 2, 22.9, 0.1, 0.2, 0.0, 0.0,
                                     0.3, 0.6, 0.0, 0.0));
