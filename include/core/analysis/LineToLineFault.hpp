...
class LineToLineFault : public FaultStrategy {
 public:
  FaultReport calculate(const models::PowerSystem& system,
    ...
    std::complex<double> z1 = zbus(faultIdx, faultIdx);
    std::complex<double> z2 = z1;

    std::complex<double> totalImpedance = z1 + z2;
    ...
    double currentRatio = std::sqrt(3.0) / magnitude;
